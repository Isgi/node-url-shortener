...
// Set record
RedisModel.prototype.set = function (long_url, hash, datesObject, cNew, callback) {
  var self = this;
  this.findUrl(long_url, function (err, reply) {
    if (err) {
      callback(500);
      self.db.quit();
    } else if (reply && (cNew == 'false' || cNew === false)) {
      callback(null, {
        'hash' : reply,
        'long_url' : long_url
      });
      ...
      if (hash) {
        ...
        function checkHash(hash) {
          self.findHash(hash, function (err, reply) {
            if (err) {
              callback(500);
            } else if (reply && 'url' in reply) {
              i++;
              function increment_last(v) {
                var getNumber = v.replace(/[0-9]+(?!.*[0-9])/, function(match) {
                  return parseInt(match, 10)+1;
                });
                return getNumber === v ? v+1 : getNumber;
              }
              console.log(increment_last(reply.hash));
              checkHash(increment_last(reply.hash));
            } else {
              var responseRecursive = {
                'hash'     : (reply && reply.hash) || hash,
                'long_url' : long_url
              };
              console.log(responseRecursive);
              self.db.multi([
                ['set', self.kUrl(long_url), responseRecursive.hash],
                ['hmset', self.kHash(responseRecursive.hash),
                  'url', long_url,
                  'hash', responseRecursive.hash,
                ...
                if (err) {
                  ...
                  callback(null, responseRecursive);
                }
                self.db.quit();
              });
            }
